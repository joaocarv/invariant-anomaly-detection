...
class ReverseDistillationModel(nn.Module):
    ...
    def forward(self, images: Tensor) -> Tensor | list[Tensor] | tuple[list[Tensor]]:
        ...
        if self.training:
            ...
            pred_scores = self.get_pred_scores(decoder_features, encoder_features)
            anomaly_maps = self.anomaly_map_generator(encoder_features, decoder_features)
            # output = self.anomaly_map_generator(encoder_features, decoder_features)
            if self.tiler:
                anomaly_maps = self.tiler.untile(anomaly_maps)
    ...
    def get_pred_scores(self, decoder_features, encoder_features):
        output = []
        batch_size = encoder_features[0].shape[0]
        for i in range(batch_size):
            curr_ef = [ef[i:i + 1].detach() for ef in encoder_features]
            curr_df = [df[i:i + 1].detach() for df in decoder_features]
            output.append(self.loss(curr_ef, curr_df))
