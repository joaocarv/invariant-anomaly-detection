...
class Cfa(AnomalyModule):
    ...
    def training_step(self, batch: dict[str, str | Tensor], *args, **kwargs) -> STEP_OUTPUT:
        ...
        distance, _ = self.model(batch["image"])
    ...
    def validation_step(self, batch: dict[str, str | Tensor], *args, **kwargs) -> STEP_OUTPUT:
        ...
        pred_scores, anomaly_maps = self.model(batch["image"])
        # batch["anomaly_maps"] = self.model(batch["image"])
        batch["anomaly_maps"] = anomaly_maps.to(self.image_threshold.value.device)
        batch["pred_scores"] = pred_scores.to(self.image_threshold.value.device)
        return batch
