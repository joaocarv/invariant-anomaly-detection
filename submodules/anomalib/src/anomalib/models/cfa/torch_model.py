...
class CfaModel(DynamicBufferModule):
    ...
    def compute_distance(self, target_oriented_features: Tensor) -> Tensor:
        ...
        f_c = 2.0 * torch.matmul(target_oriented_features.float(), (self.memory_bank.float().to(features.device)))
    ...
    def forward(self, input_tensor: Tensor) -> Tensor:
        ...
        if self.training:
            output = (distance, target_features)
        else:
            # output = self.anomaly_map_generator(distance=distance, scale=self.scale)
            # output = self.anomaly_map_generator.compute_score(distance=distance, scale=self.scale)
            # output = torch.mean(output, (1, 2, 3))
            anomaly_maps = self.anomaly_map_generator(distance=distance, scale=self.scale)
            pred_scores = torch.mean(distance.topk(self.num_nearest_neighbors, largest=False).values, (1, 2))
            output = (pred_scores, anomaly_maps)
        return output
